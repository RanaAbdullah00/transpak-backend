...
const { buildCausalTree, deterministicStateAtSequence } = require("../services/causalReplayEngine");
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const replay = await buildCausalTree(shipmentId, { limit: req.query?.limit });
      const atSeq = req.query?.atSequence;
      const deterministic =
        atSeq != null
          ? deterministicStateAtSequence(replay.events, atSeq)
          : null;
